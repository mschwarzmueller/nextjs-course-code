...
const err=new Error('No directory access');err.code='ENOENT';reject(err);}).on('error',reject).pipe(res).on('finish',resolve);});}function getContentType(extWithoutDot){const{mime}=_send.default;if('getType'in mime){// 2.0
return mime.getType(extWithoutDot);}// 1.0
return mime.lookup(extWithoutDot);}function getExtension(contentType){const{mime}=_send.default;if('getExtension'in mime){// 2.0
return mime.getExtension(contentType);}// 1.0
